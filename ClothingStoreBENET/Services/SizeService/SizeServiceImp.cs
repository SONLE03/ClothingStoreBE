...
namespace FurnitureStoreBE.Services.ProductService.SizeService
{
    public class SizeServiceImp : ISizeService
    {
        ...
        public async Task<PaginatedList<SizeResponse>> GetAllColors(PageInfo pageInfo)
        {
            var colorQuery = _dbContext.Sizes
                .Where(b => !b.IsDeleted)
                .OrderByDescending(b => b.CreatedDate)
                .ProjectTo<ColorResponse>(_mapper.ConfigurationProvider);
            var count = await _dbContext.Colors.CountAsync();
            return await Task.FromResult(PaginatedList<SizeResponse>.ToPagedList(colorQuery, pageInfo.PageNumber, pageInfo.PageSize));
        }
        public async Task<SizeResponse> CreateColor(ColorRequest colorRequest)
        {
            ...
            try
            {
                ...
                return _mapper.Map<SizeResponse>(color);
            }
            catch
            {
        ...
        public async Task<SizeResponse> UpdateColor(Guid id, ColorRequest colorRequest)
        {
